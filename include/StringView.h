...
class StringView {
   public:
    ...
    constexpr StringView(const char* cstr)
        : m_chars(cstr), m_size(string_length(cstr)) {}
   ...
   private:
    /// Caution: The given const char* still has to be null-terminated exactly
    /// at the given size, otherwise behaviour of StringView isn't defined.
    constexpr StringView(const char* cs, std::size_t sz)
        : m_chars(cs), m_size(sz) {}

   public:
    inline constexpr bool is_valid() const { return m_chars != nullptr; }
    inline constexpr bool is_empty() const {
        return m_size == 0 || m_chars[0] == 0;
    }
    ...
    friend constexpr StringView operator""_sv(const char* cstr,
                                              std::size_t size);
   ...
   private:
    inline constexpr bool equals(const char* str, std::size_t size) const {
        ...
        if (m_chars == nullptr && str == nullptr) return true;
        // if other is larger than our size we aren't equal. If we didn't have
        // this check, strncmp would return 0 (equal) for example for:
        //  strncmp("Hello", "Hello, World", strlen("Hello")), since "Hello"
        //  would have size 5, and they equal that far.
        // Similarly, if it's shorter, they aren't equal. If we're going to
        // check length we might as well check this as well and potentially save
        // ourselves a strncmp.
        if (size != m_size) return false;
        // Because of earlier checks we can now assume same size and that both
        // are not nullptr.
